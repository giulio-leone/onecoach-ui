...
> = {
  ...
  'gemini-cli': {
    description: 'Gemini 3 via Gemini CLI OAuth (usa sottoscrizione Gemini Code Assist)',
    icon: Sparkles,
    color: 'text-cyan-400',
...
export function ProviderApiKeysSection({ configs }: ProviderApiKeysProps) {
  ...
  const testConnection = async (provider: ProviderName) => {
    ...
    // OAuth providers (gemini-cli) use local credentials, no API key needed
    const isOAuthProvider = provider === 'gemini-cli';

    // Fail if no key or disabled (but OAuth providers don't need a key)
    if (!isOAuthProvider && !current?.hasKey && !current?.newApiKey) {
