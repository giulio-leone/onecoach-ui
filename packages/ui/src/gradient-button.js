...
export function GradientButton({ label, loading, variant = 'primary', className, textClassName, icon, disabled, children, onClick, onPress, ...props }) {
    ...
    return (_jsx("button", { type: "button", disabled: isDisabled, onClick: handleClick, className: cn('relative overflow-hidden rounded-lg shadow-md', 
        // Shadow color matches primary/secondary
        variant === 'primary' ? 'shadow-violet-500/20' : 'shadow-fuchsia-500/20', 'flex flex-row items-center justify-center gap-2', 'transition-all duration-200', 'hover:shadow-lg', variant === 'primary' ? 'hover:shadow-violet-500/30' : 'hover:shadow-fuchsia-500/30', 'active:scale-[0.98]', 'disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:shadow-md', 'min-h-[2.75rem] text-sm font-semibold', defaultPadding, `bg-gradient-to-br ${gradientClass}`, className), ...props, children: loading ? (_jsx(Loader2, { className: "h-4 w-4 animate-spin text-white" })) : children ? (children) : (_jsxs(_Fragment, { children: [icon && _jsx("span", { className: "flex-shrink-0", children: icon }), label && (_jsx("span", { className: cn('text-center whitespace-nowrap text-white', loading && 'opacity-80', textClassName), children: label }))] })) }));
}
