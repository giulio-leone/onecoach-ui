...
function dedupeNavigation(items) {
    ...
    return items.map(visit).filter((item) => Boolean(item));
}
...
function getNavigationItems(userRole, pathname) {
    ...
    const items = [...BASE_NAVIGATION_SECTIONS];
...
export function AppShellSidebar({ user }) {
    ...
    const setIsCollapsed = (collapsed) => setSidebarOpen(!collapsed);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(ModernSidebar, { navigation: navigation, user: effectiveUser, pathname: pathname || '', currentPath: currentPath, onSignOut: handleSignOut, isCollapsed: isCollapsed, onToggleCollapse: () => setIsCollapsed(!isCollapsed), className: "fixed inset-y-0 left-0 z-30 hidden lg:block", extraContent: extraContent }), mobileMenuOpen && (_jsxs("div", { className: "fixed inset-0 z-50 lg:hidden", children: [_jsx("div", { className: "absolute inset-0 bg-slate-900/40 backdrop-blur-sm", onClick: () => setMobileMenuOpen(false), "aria-hidden": "true" }), _jsx("div", { className: "absolute inset-y-0 left-0 w-80 max-w-[85vw] shadow-2xl", children: _jsx(ModernSidebar, { navigation: navigation, user: effectiveUser, pathname: pathname || '', currentPath: currentPath, onSignOut: handleSignOut, isMobile: true, onCloseMobile: () => setMobileMenuOpen(false), extraContent: extraContent }) })] }))] }));
}
