...
export function computeSemanticDiff(
  ...
  diff.added.forEach((path: string) => processPath(path, 'modified', { from: undefined, to: 'Added' })); // Treat deep adds as modifications to parent with "Added" value
  diff.removed.forEach((path: string) => processPath(path, 'modified', { from: 'Removed', to: undefined }));
